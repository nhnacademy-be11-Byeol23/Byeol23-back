...
public class CategoryCouponCalculationStrategy implements CouponCalculationStrategy {
    ...
    public BigDecimal calculateTargetSubtotal(CouponPolicy policy, List<OrderItemRequest> items, OrderContext context) {
        // CATEGORY 정책: 정책에 포함된 카테고리(부모 포함)에 속한 도서만 필터링
        return items.stream()
                .filter(item -> {
                    List<Long> itemCategoryIds = categoryRepository.findAllAncestorsByBookIds(List.of(item.bookId()));
                    return categoryRepository.isAnyCategoryIncludedInPolicy(itemCategoryIds, policy.getCouponPolicyId());
                })
                .map(item -> {
