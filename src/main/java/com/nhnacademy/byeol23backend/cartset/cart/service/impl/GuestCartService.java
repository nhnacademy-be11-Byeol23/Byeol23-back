...
public class GuestCartService implements CartService {
    ...
    public List<CartBookResponse> getCartBooks(CustomerIdentifier identifier) {
        Set<String> strBookIds = stringRedisTemplate.opsForZSet().reverseRange(getCartSortedSetKey(identifier.guestId()), 0, -1);
        if(strBookIds == null || strBookIds.isEmpty()) return List.of();
        List<Long> bookIds = strBookIds.stream().map(Long::valueOf).toList();

        Map<Object, Object> cartBookMap = stringRedisTemplate.opsForHash().entries(getCartHashKey(identifier.guestId()));
